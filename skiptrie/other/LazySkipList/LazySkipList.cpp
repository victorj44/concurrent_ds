...
template<typename T>
LazySkipList<T>::LazySkipList(T negInf, T posInf) :
  negInf(negInf),
  posInf(posInf)
{
...
/*template<typename T>
LazySkipList<T>::LazySkipList(int maxLevel)
{
 ...
 */
template<typename T>
int LazySkipList<T>::find(T x, Node<T> **preds, Node<T> **succs, Node<T> *startNode, 
			  int suggestedStartLevel, int *rightMoves, int *bottomMoves)
{
  int lFound = -1;
  Node<T> *pred = head;
  ...
  if (startNode != NULL)
    {
      pred = startNode;
      if (suggestedStartLevel != -1)
	startLevel = suggestedStartLevel;
      else
 ...
 */
template<typename T>
Node<T>* LazySkipList<T>::insert(T x, int randomLvl)
{
  Node<T> *ret = NULL;
...
template<typename T>
bool LazySkipList<T>::remove(T x)
{
  Node<T> *victim = NULL;
  bool isMarked = false;
  int topLevel = -1;
...
template<typename T>
bool LazySkipList<T>::contains(T x, Node<T> **contains_preds, Node<T> **contains_succs)
{
 ...
 */
template<typename T>
int LazySkipList<T>::randomLevel()
{
 ...
 */
template<typename T>
void LazySkipList<T>::printStats()
{
  int total = 0;
