 ...
 */
#include <cstdio>
#include "../xtree/hash_skip_trie.h"
...
int totalRights = 0, totalBottoms = 0, totalListHits = 0;
...
void* runOneTest(void *argIndex)
{
  ...
  while (nrep > 0)
      ...
      for (int i = 0; i < queryLen; i++)
	{
	  ...
	  int in = st->find(queryEl[p], preds, succs, &topHits, &rightMoves, &bottomMoves);
	  //printf("%d ", in);
	  localStats[in]++;
...
int main(int argc, char **argv)
{
  ...
  pthread_t *threads = new pthread_t[nthreads];
   ...
   printf("total rights = %d\ntotal bottoms = %d\n", totalRights, totalBottoms);
   printf("total list hits = %d\n", totalListHits);

   st->printTopLevelStats();
